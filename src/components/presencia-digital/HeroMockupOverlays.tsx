...
function ConsultasChartCard() {
  ...
  return (
    <motion.div
      ...
      className="absolute top-4 right-4 sm:top-3 sm:right-3 w-[120px] sm:w-[142px] rounded-lg bg-white/93 backdrop-blur-sm border border-white/70 shadow-[0_4px_18px_-6px_rgba(31,92,255,0.28)] p-2 sm:p-2.5 overflow-hidden"
    >
...
function InquiryNotificationCarousel() {
  ...
  return (
    <motion.div
      ...
      className="absolute top-4 left-4 sm:top-3 sm:left-3"
    >
...
function GoogleRankBadge() {
  ...
  return (
    <motion.div
      ...
      className="absolute bottom-4 right-4 sm:bottom-14 sm:right-3"
    >
...
function ResultBadge() {
  ...
  return (
    <motion.div
      ...
      className="absolute bottom-4 left-4 sm:-bottom-4 sm:-left-4 z-10"
    >
      <div className="flex items-center gap-1.5 sm:gap-2 bg-[#1F5CFF] text-white pl-2 sm:pl-2.5 pr-2.5 sm:pr-3 py-1.5 sm:py-2 rounded-lg shadow-md">
        <div className="flex h-5 w-5 sm:h-7 sm:w-7 items-center justify-center rounded-md bg-white/15">
          <CalendarCheck className="w-3 h-3 sm:w-4 sm:h-4" aria-hidden="true" />
        </div>
        <div>
          <p className="text-[9px] sm:text-[10px] font-semibold leading-tight whitespace-nowrap">{t('client')}</p>
          <p className="hidden sm:flex text-[8px] text-blue-100 items-center gap-1 leading-tight whitespace-nowrap">
  ...
  if (inline) {
    return (
      <motion.div
        ...
        className="inline-flex items-center gap-2 bg-[#1F5CFF] text-white pl-2.5 pr-3 py-2 rounded-lg shadow-md"
      >
...
export function HeroNotificationBadge() {
  ...
  return (
    <motion.div
      ...
      className="inline-flex items-center gap-2 rounded-lg bg-white/95 backdrop-blur-sm border border-gray-200 shadow-md px-3 py-2"
    >
      <span className="relative flex h-2 w-2 flex-shrink-0">
        <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-[#25D366] opacity-50" />
        <span className="relative inline-flex h-2 w-2 rounded-full bg-[#25D366]" />
      ...
      <div className="min-w-[88px]">
        <AnimatePresence mode="wait">
          <motion.p
            ...
            exit={prefersReducedMotion ? undefined : { opacity: 0 }}
            transition={{ duration: 0.2 }}
            className="text-[11px] font-medium text-gray-800 whitespace-nowrap leading-snug"
          >
            {activeNotification}
          </motion.p>
        </AnimatePresence>
