...
} from '@/components/crecimiento-digital/HeroMockupOverlays';
import {
...
const caseStudyImages = [
  CRECIMIENTO_DIGITAL_IMAGES.caseStudyGastroMedical,
  CRECIMIENTO_DIGITAL_IMAGES.caseStudyPhyc,
];
          ...
          >
            ...
            <div className="flex items-center gap-3 mt-auto">
              <div className="relative w-12 h-12 shrink-0 rounded-full overflow-hidden border border-white/20">
                <Image
                  src={CRECIMIENTO_DIGITAL_IMAGES.testimonialAvatar}
...
function CtaCommitment() {
  const t = useTranslations('crecimientoDigital.ctaCommitment');
        ...
        >
          {items.map((item) => (
            <motion.div
              key={item.title}
              variants={fadeInUp}
              className="group relative p-6 rounded-2xl bg-gradient-to-br from-white to-gray-50/80 border border-gray-200/60 hover:border-blue-200/80 hover:shadow-[0_8px_30px_-8px_rgba(31,92,255,0.15)] transition-all duration-300"
            >
...
const customerJourneyIcons: LucideIcon[] = [
  Lightbulb,    // What does this company offer?
  ClipboardList,  // How is their work?
  Shield,         // Can I trust this company?
  Award,          // Why choose them?
  Calendar,       // How easy is it to request a quote?
];
    ...
    <section className="relative py-24 lg:py-32 bg-gray-50 overflow-hidden">
      ...
      <Container className="relative z-10">
        ...
        >
          ...
          >
            {questions.map((question, index) => {
              const Icon = customerJourneyIcons[index] || Lightbulb;
              return (
                <div
                  key={question}
                  className="flex items-center gap-4 p-5 rounded-xl bg-white border border-gray-200/80 shadow-sm hover:shadow-md hover:border-blue-200/80 transition-all duration-300 min-h-[88px]"
          ...
          {items.map((item, index) => {
            ...
            return (
              ...
              >
                ...
                <p className="text-xs text-gray-400 mt-auto">{item.examples}</p>
              </motion.div>
            );
          })}
        </motion.div>
      ...
      <Container className="relative z-10">
        ...
        >
          <div>
            ...
            <motion.p variants={fadeInUp} className="text-base lg:text-lg text-gray-600 leading-relaxed mb-6">
              {t('description')}
            </motion.p>
            ...
            <motion.div variants={fadeInUp} className="grid grid-cols-3 gap-3">
              {stats.map((stat) => (
                <div
                  key={stat.label}
                  className="text-center p-4 rounded-xl bg-white border border-gray-200/60"
                >
                  <p className="text-xl font-bold text-[#1F5CFF] mb-1">{stat.value}</p>
                  <p className="text-xs text-gray-500">{stat.label}</p>
                </div>
              ))}
            </motion.div>
...
function DiagnosisSection() {
  ...
  return (
    <section className="relative py-16 sm:py-20 bg-gradient-to-br from-[#1F5CFF] to-blue-600 overflow-hidden">
      <div className="absolute inset-0 opacity-20 [background-image:radial-gradient(circle_at_2px_2px,rgba(255,255,255,0.3)_1px,transparent_0)] [background-size:32px_32px]" />
      <Container className="relative z-10">
        <motion.div
          initial={{ opacity: 0, y: 20 }}
        ...
        >
          <div className="grid lg:grid-cols-2 gap-8 items-center">
            <div className="text-center lg:text-left">
              <span className="inline-flex items-center gap-2 text-blue-200 font-medium text-xs uppercase tracking-[0.2em] mb-4">
                {t('eyebrow')}
              </span>
              <h2 className="text-2xl sm:text-3xl md:text-4xl font-semibold tracking-[-0.02em] text-white mb-4 leading-[1.15]">
                {t('title')}
            ...
            <div className="grid grid-cols-2 gap-3">
              {includes.map((item) => (
                <div
                  key={item}
                  className="flex items-center gap-3 p-4 rounded-xl bg-white/10 border border-white/20 backdrop-blur-sm"
                >
...
export default function CrecimientoDigitalPageContent() {
  return (
    <>
      <HeroSection />
